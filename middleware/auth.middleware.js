...
export const protectedRoute = catchError(async (req, res, next) => {
  ...
  if (user.passwordChangeAt) {
    const pwdChangedTimestamp = parseInt(user.passwordChangeAt.getTime() / 1000);
    if (pwdChangedTimestamp > decoded.iat)
      return next(new AppError('Unauthorized - Password changed after token issued', 401));
  }
...
export const allowedTo = (...roles) => {
  return catchError(async (req, res, next) => {
    if (!roles.includes(req.user.role))
      return next(new AppError(`Unauthorized - Role ${req.user.role} not allowed`, 403));
    next();
  });
};
